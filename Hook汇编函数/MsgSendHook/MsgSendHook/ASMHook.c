...
/// 存放LR寄存器值
uintptr_t ptr_t_list[10000];
int ptr_t_index = 0;

#pragma mark - 保存子程序返回地址
static void save_lr(SEL _cmd, uintptr_t lr) {
//    printf("%s \n", object_getClassName(self));
    printf("%s \n", sel_getName(_cmd));
    ptr_t_list[ptr_t_index++] = lr;
}

#pragma mark - 恢复LR寄存器值
uintptr_t resume_lr(SEL _cmd, uintptr_t lr) {
    ptr_t_index = ptr_t_index > 0 ? ptr_t_index-1 : 0;
    return ptr_t_list[ptr_t_index];
}
...
void hookStart(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        //使用fishhook替换函数指针
        struct rebinding reb = {"objc_msgSend", hook_objc_msgSend, (void *)&orig_objc_msgSend};
